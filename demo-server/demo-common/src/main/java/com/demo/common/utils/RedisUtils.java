...
public class RedisUtils {
	...
	private static final int DefaultTime = Integer.valueOf(60*1000);
	...
	public String getSetCollections(String key) {
		String result = JsonUtils.json(redisTemplate.opsForSet().members(key));
	...
	public String getMapCollections(String key) {
		return JsonUtils.json(redisTemplate.opsForHash().entries(key));
	}
	...
	public String getListStartEnd(String key, int start, int end) {
		String result = JsonUtils.json(redisTemplate.opsForList().range(key, start, end));
		return result.substring(1, result.length() - 1);
	}
	...
	/** 判断key是否存在 */
	public boolean exitsKey(final String key) {
